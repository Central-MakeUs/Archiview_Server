...
각 도메인 모듈 내부는 다음과 같은 **전통적인 레이어드 구조**를 따릅니다.
...
도메인 모델은 **Aggregate 단위로 명확히 경계를 설정**했습니다.

예시 (Post 도메인):

- `Post` : Aggregate Root
- `PostPlace` : Post 내부 Entity
- Place는 Post 없이 단독으로 존재할 수 없음
...
조회 요구사항이 복잡해짐에 따라 **쓰기(Command)와 읽기(Query)를 분리**했습니다.
...
도메인 간 직접 의존을 제거하기 위해 **이벤트 기반 통신**을 사용합니다.
...
이 구조는 향후 Kafka, SQS 등 **메시지 브로커로 자연스럽게 확장**할 수 있습니다.

---

## 🔌 Hexagonal Architecture – 선택적 차용

이 프로젝트는 **순수한 Hexagonal Architecture는 사용하지 않습니다.**

대신, **교체 가능성이 높은 지점에만 Port 개념을 선택적으로 도입**했습니다.
