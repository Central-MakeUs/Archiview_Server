...
public class ArchiverPostQueryController {
    ...
    public ResponseEntity<ApiResponse<zero.conflict.archiview.user.dto.ArchiverProfileDto.Response>> getMyProfile(
            @Parameter(hidden = true) @AuthenticationPrincipal CustomOAuth2User oAuth2User) {

        // TODO: Service 연동
        return ResponseEntity
                .ok(ApiResponse.success(
                        zero.conflict.archiview.user.dto.ArchiverProfileDto.Response.builder().build()));
    }

    @Operation(summary = "아카이버 공개 프로필 조회", description = "특정 아카이버의 공개된 프로필 정보를 조회합니다.")
    @GetMapping("/{archiverId}/profile")
    public ResponseEntity<ApiResponse<zero.conflict.archiview.user.dto.ArchiverProfileDto.Response>> getArchiverProfile(
