...
public class EditorPostQueryService {
                ...
                Place place = placeRepository.findById(placeId)
                                .orElseGet(() -> ownPostPlaces.stream()
                                                ...
                                                .filter(candidate -> candidate != null
                                                                && placeId.equals(candidate.getId()))
                                                .findFirst()
                                                .orElse(null));
                if (place == null) {
        ...
        private Comparator<Map.Entry<Long, List<PostPlace>>> getUploadedPlaceComparator(
                        PlaceSort sort) {
                Comparator<Map.Entry<Long, List<PostPlace>>> baseComparator = switch (sort) {
                        ...
                        case CREATED -> Comparator.comparing(
                                        (Map.Entry<Long, List<PostPlace>> entry) -> getLatestCreatedAt(
                                                        entry.getValue()),
                                        Comparator.nullsLast(Comparator.naturalOrder()));
                };
                ...
                return baseComparator
                                .reversed()
                                .thenComparing(Map.Entry<Long, List<PostPlace>>::getKey,
